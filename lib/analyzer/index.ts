...
export type AnalysisResult = { file: { name: string; size: number }; metadata: PluginMetadata; archive: { entryCount: number; classCount: number; resourceCount: number }; classes: ClassInventory; security: SecurityReport };
...
export async function analyzePluginJar(file: File): Promise<AnalysisResult> {
  ...
  return { file: { name: file.name, size: file.size }, metadata, archive: { entryCount: paths.length, classCount: paths.filter((path) => path.endsWith(".class")).length, resourceCount: paths.filter((path) => !path.endsWith(".class")).length }, classes: buildInventory(parsedClasses), security };
}
